...
clean_df = joined_df.withColumn(
    ...
    "CRSDepTime", (col("CRSDepTime") / 100).cast("integer")
).withColumn(
...
print(f">> Model AUROC (Area Under ROC): {auroc:.4f}")
print(f">> Categoricals encoded via Pipeline architecture.\n")


# Add this right before spark.stop()
model_export_path = "airline_delay_model"
print(f"Saving model to {model_export_path}...")
pipeline_model.write().overwrite().save(model_export_path)
